 ...
 */
export function resolveDataPath(relativePath: string, importMetaUrl: string): URL {
...
export function readJsoncSync(filePath: string | URL): any {
  const pathString = filePath instanceof URL ? fileURLToPath(filePath) : filePath;
 ...
 */
export function writeJsoncSync(filePath: string | URL, data: any, header?: string): void {
  const pathString = filePath instanceof URL ? fileURLToPath(filePath) : filePath;
