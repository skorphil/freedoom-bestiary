...
export class SpritesheetsCollection {
  private models: Record<CharacterCode, Record<string, Spritesheet>> = {} as any;
  ...
  constructor(data: SpritesheetsMap, characters?: CharactersMap, contributors?: ContributorsMap) {
    if (contributors) {
    ...
    for (const [code, sheets] of Object.entries(data)) {
      ...
      for (const [id, sheetData] of Object.entries(sheets)) {
        ...
        this.models[characterCode][id] = new Spritesheet(characterCode, sheetData, getMeta);
      }
    }
  }
  ...
  getLatest = (code: CharacterCode): Spritesheet | undefined => {
    ...
    return [...history].sort((a, b) => 
      new Date(b.data.commitDate).getTime() - new Date(a.data.commitDate).getTime()
    )[0];
  }
  ...
  getAuthorsWithRelations = (sheet: Spritesheet): { name: string; relation?: string }[] => {
  ...
  getAuthorContributions = (authorName: string): { code: CharacterCode; sheet: Spritesheet }[] => {
    ...
    return contributions.sort((a, b) => 
      new Date(b.sheet.data.commitDate).getTime() - new Date(a.sheet.data.commitDate).getTime()
    );
  }
}
