...
export async function loader({ params }: Route.LoaderArgs) {
	...
	const contributor = Object.values(allContributors).find(
		(c) => c.name === name,
	);
	...
	return (
		<>
			...
			<div style={{ marginBottom: "2rem" }}>
				<h1 style={{ fontSize: "40px", marginBottom: "0.5rem" }}>
					Author: {name}
				</h1>
				<p>
					Contributed to {contributions.length} sprite version
			...
			<div className={styles.characterGrid}>
				{contributions.map(
					({ code, sheet, character, authorsWithRelations }) => {
						const authorRelation = authorsWithRelations.find(
							(a) => a.name === name,
						)?.relation;
						return (
							<div
								key={`${code}-${sheet.commitSha}`}
								...
								<div className={styles.characterDetails}>
									...
									{authorRelation && (
										<div className={styles.metaGroup}>
											...
											<div className={styles.metaValue}>{authorRelation}</div>
										</div>
									)}
									...
									<div className={styles.metaGroup}>
										...
										>
											{sheet.commitUrl.includes("/attic/")
												? "attic"
												: "freedoom"}
										</div>
									</div>
