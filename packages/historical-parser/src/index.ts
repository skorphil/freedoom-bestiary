...
async function loadCodesFromSpritesJson(): Promise<string[]> {
	...
	try {
		...
		if (!(await file.exists())) {
			console.warn(
				`Metadata file not found at: ${spritesPath}. Using fallback codes.`,
			);
			return fallbackCodes();
		}
		const list = (await file.json()) as Array<{
			spriteCode?: string;
			doomName?: string;
		}>;
...
function toParsedSnapshot(s: CharacterVersionSnapshot): ParsedSnapshot {
	...
	return {
		...
		contributions: (s.authors || []).map((a) => {
			if (!a.contributorId) {
				console.warn(
					`Empty contributorId for author ${a.name} in commit ${s.commitSha}`,
				);
				throw new Error(
					`Missing contributorId for author ${a.name} in commit ${s.commitSha}. This should not happen.`,
				);
			}
			return {
		...
		sprites: (s.sprites || []).map((sp, idx) => {
			return {
				...
				contributions: (sp.spriteAuthors || []).map((sa) => {
					if (!sa.contributorId) {
						console.warn(
							`Empty contributorId for sprite author ${sa.name} in sprite ${sp.name} in commit ${s.commitSha}`,
						);
						throw new Error(
							`Missing contributorId for sprite author ${sa.name} in sprite ${sp.name} in commit ${s.commitSha}. This should not happen.`,
						);
					}
					return {
...
export async function runAll(opts: RunOptions = {}) {
	...
	if (opts.write) {
		for (const code of codes) {
			...
			const combined = comb.combine(
				freedoomResults[code] ?? [],
				atticResults[code] ?? [],
			);

			console.debug(
				`runAll: storing ${combined.spriteVersions.length} versions for ${code} via repository`,
			);
			...
			for (const v of chronologicalVersions) {
				...
				try {
					...
					if (
						...
						console.error(
							`Error appending snapshot for ${code} at ${v.commitSha}:`,
							e,
						);
					}
					throw e;
...
if (import.meta.main) {
	...
	for (let i = 0; i < rawArgs.length; i++) {
		...
		else if (a.startsWith("--freedoom-repo="))
			args.set("freedoomRepo", a.split("=")[1]);
		else if (a.startsWith("--attic-repo="))
			args.set("atticRepo", a.split("=")[1]);
		else if (a.startsWith("--out=")) args.set("outDir", a.split("=")[1]);
	}

	const codes = args.get("codes")
		? String(args.get("codes"))
				.split(",")
