...
export class VersionCombiner {
	...
	private resolveConflicts(frameState: Map<string, SpriteEntry>): boolean {
		...
		for (const [letter, keys] of frameGroups) {
			...
			if (angle0Keys.length > 0 && angle1to8Keys.length > 0) {
				...
				const latestEntries = allEntries.filter(
					(e) => new Date(e.lastChangedDate).getTime() === maxDate,
				);
				const maxIndex = Math.max(...latestEntries.map((e) => e.commitIndex));

				const winningEntries = latestEntries.filter(
	...
	buildVersionSnapshot(
		...
		const authorsMap = new Map<
			string,
			{ name: string; relation: string; contributorId: string }
		>();
		...
		if (authors.length === 0) {
			...
			const found = ContributorRepository.findByNameOrAlias(
				snapshot.commitAuthor,
			);
			if (found) {
				authors.push({
		...
		return {
			...
			authors:
				authors.length > 0
					? authors
					: [{ name: snapshot.commitAuthor, relation: "Commit author" }],
			sprites,
		};
	}
}
