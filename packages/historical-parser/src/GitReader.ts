...
export class GitReader {
	...
	async getTreeEntries(sha: string, folderPath?: string): Promise<TreeEntry[]> {
		...
		console.debug(
			"GitReader.getTreeEntries: running git ls-tree for",
			sha,
			"folderPath=",
			folderPath,
		);
	...
	private async lazyInitBatch(): Promise<void> {
		...
		this.batchChild = Bun.spawn(
			["git", "-C", this.repoPath, "cat-file", "--batch"],
			{
				stdin: "pipe",
				stdout: "pipe",
				stderr: "pipe",
		...
		if (this.batchChild.stdout)
			this.batchStdoutReader = this.batchChild.stdout.getReader();
	...
	private async run(args: string[]): Promise<string> {
		...
		console.debug(
			"GitReader.run: command completed, output length=",
			out.length,
		);
		return out;
	}
