...
describe("VersionCombiner", () => {
	...
	test("combine should create version with correct author structure", () => {
		...
		expect(result.spriteVersions[0].authors).toEqual([
			{
				name: "Artist A",
				relation: "Original artist",
				contributorId: "unknown",
			},
		]);
		expect(result.spriteVersions[0].sprites[0].spriteAuthors).toEqual([
			{ name: "Artist A", relation: "Original artist" },
		]);
	});
